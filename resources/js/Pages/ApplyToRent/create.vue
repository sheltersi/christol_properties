<template>
    <AuthenticatedLayout>
      ...
      <div class="max-w-4xl mx-auto mt-8 p-6 bg-white rounded shadow">
        ...
        <div>
          ...
          <textarea v-model="form.occupants" class="mt-1 block w-full rounded border-gray-300" placeholder="e.g. John (30), Mary (5)"></textarea>
        </div>
        ...
        <div v-if="form.has_pets === 'yes'">
          ...
          <textarea v-model="form.pet_details" class="mt-1 block w-full rounded border-gray-300" placeholder="e.g. 2 dogs (small breeds)"></textarea>
        </div>
          </div>
  ...
  const form = useForm({
    full_name: page.props.user?.first_name,
